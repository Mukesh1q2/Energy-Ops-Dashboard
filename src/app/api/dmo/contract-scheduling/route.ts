...
import { fetchDMOContractData } from '@/lib/excel-data-helper'
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (contractType) whereClause.contract_type = { contains: contractType }
    ...
    if (contractType) filteredExcelData = filteredExcelData.filter(d => d.contract_type?.toLowerCase().includes(contractType.toLowerCase()))
    ...
    if (exportFormat === 'csv') {
      ...
      const headers = [
        'Time Period', 'Region', 'State', 'Contract Name', 'Contract Type', 
        'Scheduled MW', 'Actual MW', 'Cumulative MW'
      ]
      ...
      return new NextResponse(csvRows.join('\n'), {
        headers: {
          ...
          'Content-Disposition': 'attachment; filename="contract-scheduling.csv"'
        }
      })
    }
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to fetch contract scheduling data',
